...
// //       Color.fromARGB(255, 204, 204, 204); // You can modify these values
// //   // static const Color primary = Color(0xFF043D72); // You can modify these values
// //   // static const Color scaffoldColor = Color(0xff0F0F0F);
// //   // static const Color scaffoldColor = Color(0xff101110);
// //   static const Color scaffoldColor = Color(0xff161616);
// //   // static const Color containers = Color(0xff262626);
// //   static const Color containers = Color(0xff2b2b2b);
...
class ColorManager {
  ...
  static const Color _defaultPrimary = Color(0xFFe72d2d);
  static const Color _defaultSecondary = Color(0xFFec9d22);
  ...
  // Initialize and load cached colors
  Future<void> initialize() async {
    await _loadCachedColors();
  }

  Future<void> _loadCachedColors() async {
    final savedPrimary = await SharedPreferencesService.getOrganizationPrimaryColor();
    final savedSecondary = await SharedPreferencesService.getOrganizationSecondaryColor();
    
    if (savedPrimary != null) {
      _primary = Color(savedPrimary);
    }
    if (savedSecondary != null) {
      _secondary = Color(savedSecondary);
    }
  }

  // Update colors (when organization loads)
  void updateColors(Color primary, Color secondary) {
    _primary = primary;
    _secondary = secondary;
    
    // Save to cache
    SharedPreferencesService.saveOrganizationColors(
      primary.value, 
      secondary.value
    );
  }
  ...
  void resetToDefaults() {
    ...
    _secondary = _defaultSecondary;
  }

  // Derived colors
  Color get primaryLight => _primary.withOpacity(0.7);
  Color get primaryDark => _primary.withAlpha(200);
  Color get secondaryLight => _secondary.withOpacity(0.7);
  Color get secondaryDark => _secondary.withAlpha(200);
...
class AppColors {
  static ColorManager get _colors => ColorManager();

  // These will use cached organization colors or defaults
  ...
  static LinearGradient get accentGradient => _colors.accentGradient;
}
