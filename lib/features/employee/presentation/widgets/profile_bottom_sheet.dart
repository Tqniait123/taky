...
class _ProfileBottomSheetState extends State<ProfileBottomSheet>
  ...
  void _checkForChanges() {
    ...
    final jobTitleChanged =
        _jobTitleController.text.trim() != (widget.user.jobTitle ?? '');

    setState(() {
      _hasChanges = nameChanged || phoneChanged || jobTitleChanged;
    });
  }
  ...
  void _updateProfile(String locale) async {
    ...
    try {
      // Update user data including job title
      final updatedData = {
  ...
  void _showDeleteAccountConfirmation(String locale) {
    showDialog(
      ...
      barrierDismissible: false,
      builder: (context) {
        return EmployeeDeleteAccountDialog(
          locale: locale,
          user: widget.user,
  ...
  Widget _buildEmailRow(String icon, String email, int delay, locale) {
    return TweenAnimationBuilder<double>(
      tween: Tween(begin: 0.0, end: 1.0),
      duration: Duration(milliseconds: 600 + delay),
      curve: Curves.easeOutBack,
...
// Separate widget for Employee Delete Account Dialog
class EmployeeDeleteAccountDialog extends StatefulWidget {
  final String locale;
  final EmployeeUserModel user;
  ...
  const EmployeeDeleteAccountDialog({
    super.key,
  ...
  @override
  State<EmployeeDeleteAccountDialog> createState() =>
      _EmployeeDeleteAccountDialogState();
}

class _EmployeeDeleteAccountDialogState
    extends State<EmployeeDeleteAccountDialog> {
